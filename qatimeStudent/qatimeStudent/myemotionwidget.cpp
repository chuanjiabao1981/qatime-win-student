...
// 	initWindow();
	...
	QPalette p = palette();
	p.setColor(QPalette::Window, QColor(254, 255, 255));
	setPalette(p);
...
void MyEmotionWidget::initTableWidget()
{
	...
	{
		this->setFixedHeight(m_emotionSize.height()*m_maxRow);
		this->setFixedWidth(m_emotionSize.width()*m_tableColumn + 20);
	}
	else
	{
		this->setFixedHeight(m_emotionSize.height()*m_tableRow);
...
void MyEmotionWidget::setBarFocus()
{
	this->verticalScrollBar()->setFocus();
}